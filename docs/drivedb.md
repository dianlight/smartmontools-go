...
To update to a specific upstream commit manually:

1. Download the target `drivedb.h`:
```bash
COMMIT=<upstream-commit-sha>
curl -fsSL "https://raw.githubusercontent.com/smartmontools/smartmontools/${COMMIT}/lib/drivedb.h" \
  -o backends/exec/drivedb.h
```
2. Update `backends/exec/drivedb_version.go` with the new commit SHA and date.
3. Rebuild: `go build`

### Embedded Version Constants

The file `backends/exec/drivedb_version.go` (generated by the update workflow) exposes the
upstream provenance of the embedded file:

```go
import smartmontools "github.com/dianlight/smartmontools-go"
